...
# e agora possui uma loja de conveniências. Faça um programa que implemente uma caixa registradora
...
while True:
    ...
    soma = cont = 0
    while True:
        compra = float(input('Digita o valor do produto ou 0 para sair: '))
        if compra == 0:
            break
        else:
            lista.append(compra)
    ...
    for cada in lista:
        soma += cada
        cont += 1
        print(f'Produto {cont}: R$ {cada:05.2f}')

    print(f'Total: R$ {soma:05.2f}')
